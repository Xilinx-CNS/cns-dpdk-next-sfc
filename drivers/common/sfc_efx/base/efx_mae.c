	...
	__out				efx_mae_limits_t *emlp)
{
	...
	emlp->eml_max_n_counters = maep->em_max_counters;
	...
	__out				efx_mport_sel_t *mportp)
{
	...
	EFX_POPULATE_DWORD_4(dword,
	    ...
	    MAE_MPORT_SELECTOR_FUNC_INTF_ID, MAE_MPORT_SELECTOR_CALLER_INTF,
	...
	__in				const efx_mae_actions_t *right)
{
	...
	/*
	 * Do not compare resource IDs since the caller may ask to compare a
	 * spec that doesn't have resource IDs filled in with a spec of an
	 * active action set (the same fields might contain valid IDs there).
	 *
	...
	__out				efx_mae_aset_id_t *aset_idp)
{
	...
	if ((spec->ema_actions & (1U << EFX_MAE_ACTION_ENCAP)) != 0) {
		eh_id = spec->ema_rsrc.emar_eh_id.id;
	...
	__out_opt			uint32_t *gen_countp)
{
	...
	EFSYS_ASSERT(n_allocated <= n_counters);
...
fail4:
	EFSYS_PROBE(fail4);
	(void) efx_mae_counters_free(enp, n_allocated, countersp, NULL);
fail3:
	EFSYS_PROBE(fail3);
fail2:
	EFSYS_PROBE(fail2);
