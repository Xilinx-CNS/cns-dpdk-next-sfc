	...
	__out_ecount(field_ncaps)	efx_mae_field_cap_t *field_caps)
{
	...
	req.emr_out_buf = payload;
	req.emr_out_length =
	    MC_CMD_MAE_GET_AR_CAPS_OUT_LEN(field_ncaps);
	...
	__out				efx_mae_rc_handle_t *handlep)
{
	...
	EFX_STATIC_ASSERT(EFX_MAE_HANDLE_NULL ==
	    MCDI_ID(MC_CMD_MAE_AR_CLASS_REG_OUT,
	...
	MCDI_IN_SET_DWORD(req, MAE_AR_CLASS_REG_IN_PRIO,
	    spec->emms_prio);
	...
	handle.h = MCDI_OUT_DWORD(req,
			    MAE_AR_CLASS_REG_OUT_ARC_HANDLE);
	...
	__in				efx_mae_rc_handle_t *handlep)
{
	...
	MCDI_IN_SET_DWORD(req, MAE_AR_CLASS_UNREG_IN_ARC_HANDLE,
			  handlep->h);
	efx_mcdi_execute(enp, &req);
	...
	if (MCDI_OUT_DWORD(req,
			   MCDI_ID(MAE_AR_CLASS_UNREG_OUT,
				   UNREGD_ARC_HANDLE)) != handlep->h) {
	...
	__out				efx_mport_id_t *mport_idp)
{
	...
	memset(mport_idp, 0, sizeof (*mport_idp));
	mport_idp->id = dword.ed_u32[0];
	...
	__in_bcount(arg_size)		const uint8_t *arg)
{
	...
	EFX_STATIC_ASSERT(EFX_MAE_ACTION_FLAG + 1 ==
		EFX_MAE_ACTION_MARK);
	EFX_STATIC_ASSERT(EFX_MAE_ACTION_MARK + 1 ==
		EFX_MAE_ACTION_DELIVER);
