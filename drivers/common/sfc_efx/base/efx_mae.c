...
efx_mae_read_mport_journal_batch(
	__in				efx_nic_t *enp,
	__in				efx_mae_read_mport_journal_cb *cb,
	__in				void *cb_data,
	__out				uint32_t *morep)
{
	...
	if (entry_sz < MAE_MPORT_DESC_LEN ||
	    req.emr_out_length_used != 12 + n_entries * entry_sz) {
		rc = EINVAL;
		goto fail4;
	}
	...
	for (i = 0; i < n_entries; i++) {
		...
		desc.emd_can_receive_on = MCDI_STRUCT_DWORD_FIELD(entry_buf,
		    MAE_MPORT_DESC_V2_FLAGS, MAE_MPORT_DESC_V2_CAN_RECEIVE_ON);
		desc.emd_can_deliver_to = MCDI_STRUCT_DWORD_FIELD(entry_buf,
		    MAE_MPORT_DESC_V2_FLAGS, MAE_MPORT_DESC_V2_CAN_DELIVER_TO);
		desc.emd_can_delete = MCDI_STRUCT_DWORD_FIELD(entry_buf,
		    MAE_MPORT_DESC_V2_FLAGS, MAE_MPORT_DESC_V2_CAN_DELETE);
		desc.emd_zombie = MCDI_STRUCT_DWORD_FIELD(entry_buf,
		    MAE_MPORT_DESC_V2_FLAGS, MAE_MPORT_DESC_V2_IS_ZOMBIE);
		...
		case EFX_MPORT_TYPE_VNIC:
			...
			if (desc.emd_vnic.ev_client_type !=
			    ...
			    break;
			fn_intf = MCDI_STRUCT_DWORD(entry_buf,
			    MAE_MPORT_DESC_V2_VNIC_FUNCTION_INTERFACE);
			desc.emd_vnic.ev_fn_interface =
			    MCDI_STRUCT_DWORD(entry_buf,
			        MAE_MPORT_DESC_V2_VNIC_FUNCTION_INTERFACE);
	...
	__in				void *cb_data)
{
	...
	do {
		rc = efx_mae_read_mport_journal_batch(enp, cb, cb_data, &more);
		if (rc != 0)
			goto fail2;
	} while (more != 0);
