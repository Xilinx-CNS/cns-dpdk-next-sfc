...
{
	const efx_nic_dma_regioned_t *regioned;
	...
	regioned = &enp->en_dma.end_u.endu_regioned;

	for (i = 0; i < regioned->endr_num; ++i) {
		efsys_dma_addr_t offset;

		region = &regioned->endr_regions[i];
...
efx_nic_dma_config_regioned_add_region(
	...
	__out		const efx_nic_dma_region_t **regionp)
{
	efx_nic_dma_regioned_t *regioned;
	...
	__in		efx_nic_t *enp)
{
	...
	rc = efx_mcdi_set_nic_addr_regions(enp,
	    &enp->en_dma.end_u.endu_regioned);
	if (rc != 0)
		goto fail1;
	...
	__out		efsys_dma_addr_t *nic_addrp)
{
	...
	case EFX_NIC_DMA_ADDR_MCDI_BUF:
		 ...
		 * be really mapped using MCDI because of chicken-and-egg
		 * problem.
		 */
		*nic_addrp = trgt_addr;
		break;
	...
	__out		efsys_dma_addr_t *nic_addrp)
{
	...
	case EFX_NIC_DMA_ADDR_MAC_STATS_BUF:
		/*
		 * MC cares about MCDI buffer (since it cannot be really
		 * mapped because of chicken-and-egg problem) and MAC
		 * stats buffer address mapping itself.
		 */
		*nic_addrp = trgt_addr;
		return (0);
