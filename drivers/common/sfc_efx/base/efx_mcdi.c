	...
	__out		efx_nic_dma_regioned_t *regioned)
{
	...
	num = MC_CMD_GET_DESC_ADDR_REGIONS_OUT_REGIONS_NUM(
...
/* FIXME */
#define DESC_ADDR_REGION_DESC_ADDR_BASE_LO_LBN \
		DESC_ADDR_REGION_DESC_ADDR_BASE_LBN
#define DESC_ADDR_REGION_DESC_ADDR_BASE_LO_WIDTH \
		(DESC_ADDR_REGION_DESC_ADDR_BASE_WIDTH / 2)
#define DESC_ADDR_REGION_DESC_ADDR_BASE_HI_LBN \
		(DESC_ADDR_REGION_DESC_ADDR_BASE_LO_LBN + \
		 DESC_ADDR_REGION_DESC_ADDR_BASE_LO_WIDTH)
#define DESC_ADDR_REGION_DESC_ADDR_BASE_HI_WIDTH \
		(DESC_ADDR_REGION_DESC_ADDR_BASE_WIDTH / 2)
#define DESC_ADDR_REGION_TRGT_ADDR_BASE_LO_LBN \
		DESC_ADDR_REGION_TRGT_ADDR_BASE_LBN
#define DESC_ADDR_REGION_TRGT_ADDR_BASE_LO_WIDTH \
		(DESC_ADDR_REGION_TRGT_ADDR_BASE_WIDTH / 2)
#define DESC_ADDR_REGION_TRGT_ADDR_BASE_HI_LBN \
		(DESC_ADDR_REGION_TRGT_ADDR_BASE_LO_LBN + \
		 DESC_ADDR_REGION_TRGT_ADDR_BASE_LO_WIDTH)
#define DESC_ADDR_REGION_TRGT_ADDR_BASE_HI_WIDTH \
		(DESC_ADDR_REGION_TRGT_ADDR_BASE_WIDTH / 2)

		region_info->endr_inuse = B_FALSE;
		region_info->endr_nic_base =
		    ((efsys_dma_addr_t)EFX_XWORD_FIELD(regions[i],
			DESC_ADDR_REGION_DESC_ADDR_BASE_HI) <<
			DESC_ADDR_REGION_DESC_ADDR_BASE_LO_WIDTH) |
		    (efsys_dma_addr_t)EFX_XWORD_FIELD(regions[i],
			DESC_ADDR_REGION_DESC_ADDR_BASE_LO);
		region_info->endr_trgt_base =
		    ((efsys_dma_addr_t)EFX_XWORD_FIELD(regions[i],
			DESC_ADDR_REGION_TRGT_ADDR_BASE_HI) <<
			DESC_ADDR_REGION_TRGT_ADDR_BASE_LO_WIDTH) |
		    (efsys_dma_addr_t)EFX_XWORD_FIELD(regions[i],
			DESC_ADDR_REGION_TRGT_ADDR_BASE_LO);
	...
	__in		const efx_nic_dma_regioned_t *regioned)
{
	...
	req.emr_in_buf = payload;
	req.emr_in_length =
	    MC_CMD_SET_DESC_ADDR_REGIONS_IN_LEN(regioned->endr_num);
	req.emr_out_buf = payload;
