	...
	__out		efx_nic_dma_region_info_t *endrip)
{
	...
	EFSYS_KMEM_ALLOC(enp->en_esip,
	    nregions * sizeof (endrip->endr_regions[0]),
	    endrip->endr_regions);
	if (endrip->endr_regions == NULL) {
		rc = ENOMEM;
		goto fail3;
	}

	endrip->endr_count = nregions;
	for (i = 0; i < nregions; ++i) {
		efx_nic_dma_region_t *region_info;

		region_info = &endrip->endr_regions[i];
	...
	__in		const efx_nic_dma_region_info_t *endrip)
{
	...
	req.emr_in_buf = payload;
	req.emr_in_length =
	    MC_CMD_SET_DESC_ADDR_REGIONS_IN_LEN(endrip->endr_count);
	req.emr_out_buf = payload;
