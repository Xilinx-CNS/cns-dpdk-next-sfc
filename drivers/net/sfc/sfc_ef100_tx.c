...
sfc_ef100_tx_reap(struct sfc_ef100_txq *txq)
{
	...
	pending += sfc_ef100_tx_process_events(txq);
			       ...
			       efx_oword_t *tx_desc)
{
	EFX_POPULATE_OWORD_4(*tx_desc,
			     ESF_GZ_TX_SEND_TYPE, ESE_GZ_TX_SEND_DESC,
			     ESF_GZ_TX_SEND_BUF_ADDR, addr,
		    ...
		    struct sfc_dp_txq **dp_txqp)
{
	...
	txq->evq_hw_index = info->hw_index;
...
static bool
sfc_ef100_tx_qtx_ev(struct sfc_dp_txq *dp_txq, __rte_unused unsigned int id)
{
	__rte_unused struct sfc_ef100_txq *txq = sfc_ef100_txq_by_dp_txq(dp_txq);
...
sfc_ef100_tx_qdesc_npending(struct sfc_ef100_txq *txq)
{
	...
	while (sfc_ef100_tx_get_event(txq, &tx_ev))
		anew_done = EFX_QWORD_FIELD(tx_ev, ESF_GZ_EV_NUM_DESCS);
...
static int
sfc_ef100_tx_qdesc_status(struct sfc_dp_txq *dp_txq,
			 uint16_t offset)
{
	struct sfc_ef100_txq *txq = sfc_ef100_txq_by_dp_txq(dp_txq);
	unsigned int npending = sfc_ef100_tx_qdesc_npending(txq);
