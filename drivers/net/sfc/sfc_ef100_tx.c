			       ...
			       efx_oword_t *tx_desc)
{
	...
	case PKT_TX_UDP_CKSUM:
		partial_en = 1;
		partial_cksum = offsetof(struct udp_hdr, dgram_cksum);
		break;
	case PKT_TX_TCP_CKSUM:
		partial_en = 1;
		partial_cksum = offsetof(struct tcp_hdr, cksum);
		break;
	default:
		partial_en = 0;
		partial_cksum = 0;
	...
	EFX_POPULATE_OWORD_7(*tx_desc,
			     ...
			     ESF_GZ_TX_SEND_CSO_PARTIAL_START, l4_offset,
			     ESF_GZ_TX_SEND_CSO_PARTIAL_CSUM, partial_cksum,
			      ...
			      efx_oword_t *tx_desc)
{
	EFX_POPULATE_OWORD_3(*tx_desc,
			     ESF_GZ_TX_SEND_TYPE, ESE_GZ_TX_SEG_DESC,
			      ...
			      efx_oword_t *tx_desc)
{
	...
	EFX_POPULATE_OWORD_10(*tx_desc,
			      ESF_GZ_TX_TSO_TYPE, ESE_GZ_TX_TSO_DESC,
...
sfc_ef100_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts)
{
	...
	for (pktp = &tx_pkts[0], pktp_end = &tx_pkts[nb_pkts];
		...
		if (m_seg->ol_flags & PKT_TX_TCP_SEG) {
			...
			id = added++ & txq->ptr_mask;
			sfc_ef100_tx_qdesc_send_create(seg_addr,
...
sfc_ef100_tx_qdesc_npending(struct sfc_ef100_txq *txq)
{
	...
	while (sfc_ef100_tx_get_event(txq, &tx_ev))
		npending += EFX_QWORD_FIELD(tx_ev, ESF_GZ_EV_NUM_DESCS);
