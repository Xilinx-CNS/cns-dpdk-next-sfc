		    ...
		    struct sfc_mae_counter_id *counterp)
{
	...
	rc = efx_mae_counters_alloc(sa->nic, 1, &mae_counter,
				    &generation_count);
	if (rc != 0)
	...
	p->reset.hits = p->value.hits;
	p->reset.bytes = p->value.bytes;
		    ...
		    const struct sfc_mae_counter_id *counter)
{
	...
	rc = efx_mae_counters_free(sa->nic, 1, &counter->mae_id, NULL);
	if (rc != 0)
			  ...
			  uint64_t hits, uint64_t bytes)
{
	...
	union sfc_mae_counter_value cnt_val;
	bool inuse;
	...
	cnt_val.hits = p->value.hits + hits;
	cnt_val.bytes = p->value.bytes + bytes;

	__atomic_store(&p->value.hits_bytes,
		       &cnt_val.hits_bytes, __ATOMIC_RELAXED);
}
