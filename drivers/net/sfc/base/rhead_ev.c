	...
	__in		unsigned int count)
{
	...
	EFX_POPULATE_DWORD_2(dword, ERF_GZ_EVQ_ID, eep->ee_index,
	    ERF_GZ_IDX, rptr);
	EFX_BAR_VI_WRITED(enp, ER_GZ_EVQ_INT_PRIME, eep->ee_index,
	    &dword, B_FALSE);
	...
	__in_opt	void *arg)
{
	 ...
	 */
	num_packets = EFX_QWORD_FIELD(*eqp, ESF_GZ_EV_NUM_PACKETS);
	...
	__in_opt	void *arg)
{
	 ...
	 */
	num_descs = EFX_QWORD_FIELD(*eqp, ESF_GZ_EV_NUM_DESCS);
	...
	__in_opt	void *arg)
{
	...
	/*
	 * Events on EF100 have a different layout to EF10, but
	 * all of the fields use the same encodings.
	 *
	 * Reformat the EF100 event into EF10 layout for handling.
	 */
	EFX_POPULATE_QWORD_5(ef10_mcdi_ev,
	    MCDI_EVENT_DATA, EFX_QWORD_FIELD(*eqp, EF100_MCDI_EVENT_DATA),
	    MCDI_EVENT_CONT, EFX_QWORD_FIELD(*eqp, EF100_MCDI_EVENT_CONT),
	    MCDI_EVENT_LEVEL, EFX_QWORD_FIELD(*eqp, EF100_MCDI_EVENT_LEVEL),
	    MCDI_EVENT_SRC, EFX_QWORD_FIELD(*eqp, EF100_MCDI_EVENT_PTP_DATA),
	    MCDI_EVENT_CODE, EFX_QWORD_FIELD(*eqp, EF100_MCDI_EVENT_CODE));
	ret = ef10_ev_mcdi(eep, &ef10_mcdi_ev, eecp, arg);
