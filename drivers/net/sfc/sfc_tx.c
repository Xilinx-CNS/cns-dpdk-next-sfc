	     ...
	     const struct rte_eth_txconf *tx_conf)
{
	...
	rc = sa->dp_tx->qsize_up_rings(nb_tx_desc, &hw_limits, &txq_entries,
	...
	txq->hw_index = sfc_evq_index_by_txq_sw_index(sa, sw_index);
	txq->evq = evq;
	txq->free_thresh =
		     ...
		     uint16_t nb_pkts)
{
	...
	for (i = 0; i < nb_pkts; i++) {
		...
		ret = sfc_dp_tx_prepare_pkt(tx_pkts[i], txq->max_fill_level,
				encp->enc_tx_tso_tcp_header_offset_limit,
				encp->enc_hw_tx_insert_vlan_enabled);
		if (unlikely(ret != 0)) {
			rte_errno = ret;
