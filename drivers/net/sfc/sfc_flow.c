		   ...
		   struct rte_flow *flow)
{
	...
	ethdev_rx_qid = sfc_sa2shared(sa)->ethdev_rxq_count - 1;
	rxq = sfc_rxq_ctrl_by_ethdev_qid(sa, ethdev_rx_qid);
	rxq_hw_index_min = rxq->hw_index;
	rxq_hw_index_max = 0;

	for (i = 0; i < action_rss->queue_num; ++i) {
		ethdev_rx_qid = action_rss->queue[i];

		if (ethdev_rx_qid >= sfc_sa2shared(sa)->ethdev_rxq_count)
			return -EINVAL;
	...
	for (i = 0; i < RTE_DIM(sfc_rss_conf->rss_tbl); ++i) {
		...
		unsigned int ethdev_rx_qid = action_rss->queue[i % nb_queues];
		struct sfc_rxq *rxq;
		...
		struct rte_flow_error *error)
{
	...
	if (insert && sa->state == SFC_ADAPTER_STARTED) {
