 ...
 */
#define SFC_EF100_RX_WPTR_ALIGN	8u
...
sfc_ef100_rx_qpush(struct sfc_ef100_rxq *rxq, unsigned int added)
{
	...
	RTE_BUILD_BUG_ON(SFC_RX_REFILL_BULK % SFC_EF100_RX_WPTR_ALIGN != 0);
	SFC_ASSERT(RTE_ALIGN(added, SFC_EF100_RX_WPTR_ALIGN) == added);
...
sfc_ef100_rx_prefix_to_offloads(const efx_oword_t *rx_prefix,
				struct rte_mbuf *m, uint64_t ol_mask)
{
	uint32_t l2_ptype = 0;
	uint64_t ol_flags = 0;

	/* Zero packet type is used as a marker to dicard bad packets */
	if (unlikely(EFX_OWORD_FIELD(rx_prefix[0], ESF_GZ_LEN_ERR)))
		goto done;
	if (unlikely(!EFX_OWORD_FIELD(rx_prefix[0], ESF_GZ_FCS_OK)))
		goto done;
	...
	if (EFX_TEST_OWORD_BIT(rx_prefix[0], ESF_GZ_USR_FLAG_LBN)) {
				...
				struct rte_mbuf ** const rx_pkts_end)
{
	while (rxq->ready_pkts > 0 && rx_pkts != rx_pkts_end) {
		...
		pkt_len = EFX_OWORD_FIELD(rx_prefix[0], ESF_GZ_LEN);
		...
		sfc_ef100_rx_prefix_to_offloads(rx_prefix, pkt, ~0ull);
		    ...
		    struct sfc_dp_rxq **dp_rxqp)
{
	...
	rxq->evq_prime =
		(volatile uint8_t *)info->mem_bar + ER_GZ_EVQ_INT_PRIME_OFST;
	rxq->evq_hw_index = info->hw_index;
