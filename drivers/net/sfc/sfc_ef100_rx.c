...
{
	return EFX_WORD_FIELD(class, ESF_GZ_RX_PREFIX_NT_OR_INNER_L4_CSUM) ==
		ESE_GZ_L4_CSUM_GOOD ?
...
{
	return EFX_WORD_FIELD(class, ESF_GZ_RX_PREFIX_TUN_OUTER_L4_CSUM) ==
		ESE_GZ_L4_CSUM_GOOD ?
...
sfc_ef100_rx_class_decode(const efx_word_t class, uint64_t *ol_flags)
{
	...
	if (unlikely(EFX_WORD_FIELD(class, ESF_GZ_RX_PREFIX_L2_CLASS) !=
		     ESE_GZ_L2_CLASS_E2_123VLAN))
		return 0;

	switch (EFX_WORD_FIELD(class, ESF_GZ_RX_PREFIX_L2_N_VLAN)) {
	case 0:
	...
	case ESE_GZ_TUNNEL_CLASS_NONE:
		no_tunnel = true;
		break;
	case ESE_GZ_TUNNEL_CLASS_VXLAN:
		ptype |= RTE_PTYPE_TUNNEL_VXLAN | RTE_PTYPE_L4_UDP;
		*ol_flags |= sfc_ef100_rx_tun_outer_l4_csum(class);
		break;
	case ESE_GZ_TUNNEL_CLASS_NVGRE:
		ptype |= RTE_PTYPE_TUNNEL_NVGRE;
		break;
	case ESE_GZ_TUNNEL_CLASS_GENEVE:
	...
	if (no_tunnel) {
		...
		switch (EFX_WORD_FIELD(class,
			ESF_GZ_RX_PREFIX_NT_OR_INNER_L3_CLASS)) {
		case ESE_GZ_L3_CLASS_IP4GOOD:
			ptype |= RTE_PTYPE_L3_IPV4_EXT_UNKNOWN;
			*ol_flags |= PKT_RX_IP_CKSUM_GOOD;
			break;
		case ESE_GZ_L3_CLASS_IP4BAD:
			ptype |= RTE_PTYPE_L3_IPV4_EXT_UNKNOWN;
			*ol_flags |= PKT_RX_IP_CKSUM_BAD;
			break;
		case ESE_GZ_L3_CLASS_IP6:
		...
		if (l4_valid) {
			switch (EFX_WORD_FIELD(class,
				ESF_GZ_RX_PREFIX_NT_OR_INNER_L4_CLASS)) {
			case ESE_GZ_L4_CLASS_TCP:
				ptype |= RTE_PTYPE_L4_TCP;
				*ol_flags |=
					sfc_ef100_rx_nt_or_inner_l4_csum(class);
				break;
			case ESE_GZ_L4_CLASS_UDP:
				ptype |= RTE_PTYPE_L4_UDP;
				*ol_flags |=
					sfc_ef100_rx_nt_or_inner_l4_csum(class);
				break;
			case ESE_GZ_L4_CLASS_FRAG:
				...
				struct rte_mbuf *m)
{
	...
	RTE_BUILD_BUG_ON(EFX_LOW_BIT(ESF_GZ_CLASS) % CHAR_BIT != 0);
	RTE_BUILD_BUG_ON(EFX_WIDTH(ESF_GZ_CLASS) % CHAR_BIT != 0);
	RTE_BUILD_BUG_ON(EFX_WIDTH(ESF_GZ_CLASS) / CHAR_BIT != sizeof(*class));
	class = (const efx_word_t *)((const uint8_t *)rx_prefix +
		EFX_LOW_BIT(ESF_GZ_CLASS) / CHAR_BIT);
	if (unlikely(EFX_WORD_FIELD(*class, ESF_GZ_RX_PREFIX_L2_STATUS) !=
		     ESE_GZ_L2_STATUS_OK))
		goto done;
	...
	if ((rxq->flags & SFC_EF100_RXQ_USER_MARK) &&
		...
		/* EFX_OWORD_FIELD converts little-endian to CPU */
		m->hash.fdir.hi =
			EFX_OWORD_FIELD(rx_prefix[0], ESF_GZ_USER_MARK);
	}
				...
				struct rte_mbuf ** const rx_pkts_end)
{
	while (rxq->ready_pkts > 0 && rx_pkts != rx_pkts_end) {
		...
		pkt_len = EFX_OWORD_FIELD(rx_prefix[0], ESF_GZ_LENGTH);
		...
		while ((pkt_len -= seg_len) > 0) {
			...
			pkt->nb_segs++;
			rte_pktmbuf_lastseg(pkt)->next = seg;
		}
...
sfc_ef100_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts)
{
	...
	while (rx_pkts != rx_pkts_end && sfc_ef100_rx_get_event(rxq, &rx_ev)) {
		rxq->ready_pkts = EFX_QWORD_FIELD(rx_ev, ESF_GZ_EV_NUM_PACKETS);
