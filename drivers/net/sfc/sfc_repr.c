...
sfc_repr_tx_burst(void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts)
{
	 ...
	 * instead of doing that in representors proxy.
	 * Also, it should help to avoid cache bounce.
	 * Moreover, potentially, it allows to use one multi-producer
...
sfc_repr_start(struct rte_eth_dev *dev)
{
	...
	ret = sfc_repr_proxy_start_id(srs->pf_port_id, srs->repr_id);
	if (ret == 0) {
...
sfc_repr_stop(struct rte_eth_dev *dev)
{
	...
	srs = sfc_repr_shared_by_eth_dev(dev);
	sfc_repr_proxy_stop_id(srs->pf_port_id, srs->repr_id);
...
sfc_repr_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats)
{
	...
	for (i = 0; i < dev->data->nb_rx_queues; i++) {
		...
		stats->ipackets += rxq->ipackets;
		stats->ibytes += rxq->ibytes;
	}

	for (i = 0; i < dev->data->nb_tx_queues; i++) {
		struct sfc_repr_txq *txq = dev->data->tx_queues[i];

		stats->opackets += txq->opackets;
		stats->obytes += txq->obytes;
	}
