...
{
	/*
	 * If multiple representor proxy queues are set up, not all
	 * corresponding representor queues can be setup. Skip the
	 * queues that are not set up.
	 */
 ...
 * port which is different from that is currently routed.
 */
static bool
			...
			struct sfc_repr_proxy_dp_rxq *rp_rxq)
{
	...
	for (i = rp_rxq->routed;
		...
		if (port->repr_id != rp_rxq->route_port_id &&
		    rp_rxq->route_port_id != UINT16_MAX)
			return true;
			  ...
			  struct sfc_repr_proxy_dp_rxq *rp_rxq)
{
	...
	if (rp_rxq->route_port_id != UINT16_MAX) {
...
sfc_repr_proxy_txq_info_get(struct sfc_adapter *sa, unsigned int repr_queue_id)
{
	...
	return &sas->txq_info[sa->repr_proxy.dp_txq[repr_queue_id].sw_index];
}
...
sfc_repr_proxy_txq_attach(struct sfc_adapter *sa)
{
	...
	for (i = 0; i < sfc_repr_txq_num(sas); i++) {
		int sw_index = sfc_repr_txq_sw_index(sas, i);
...
sfc_repr_proxy_txq_init(struct sfc_adapter *sa)
{
	...
	for (init_i = 0; init_i < sfc_repr_txq_num(sas); init_i++) {
...
{
	return &sfc_sa2shared(sa)->rxq_info[
				sa->repr_proxy.dp_rxq[repr_queue_id].sw_index];
}
...
static int
sfc_repr_proxy_do_start_id(struct sfc_adapter *sa,
			   struct sfc_repr_proxy_port *port)
{
	struct sfc_repr_proxy *rp = &sa->repr_proxy;
	int rc;
...
static int
sfc_repr_proxy_do_stop_id(struct sfc_adapter *sa,
			  struct sfc_repr_proxy_port *port)
...
sfc_repr_proxy_start(struct sfc_adapter *sa)
{
	...
	rp->nb_txq = sfc_repr_txq_num(sas);
	rp->nb_rxq = sfc_repr_rxq_num(sas);
	...
	TAILQ_FOREACH(port, &rp->ports, entries) {
		if (sfc_repr_proxy_port_enabled(port)) {
			rc = sfc_repr_proxy_do_start_id(sa, port);
			if (rc != 0)
				goto fail_start_id;
...
fail_start_id:
	if (last_port != NULL) {
		TAILQ_FOREACH(port, &rp->ports, entries) {
			if (sfc_repr_proxy_port_enabled(port)) {
				(void)sfc_repr_proxy_do_stop_id(sa, port);
				if (port == last_port)
					break;
			}
		}
...
sfc_repr_proxy_stop(struct sfc_adapter *sa)
{
	...
	TAILQ_FOREACH(port, &rp->ports, entries) {
		if (sfc_repr_proxy_port_enabled(port)) {
			rc = sfc_repr_proxy_do_stop_id(sa, port);
			if (rc != 0) {
			    sfc_err(sa,
