...
static int
sfc_rx_queue_setup(struct rte_eth_dev *dev, uint16_t ethdev_rx_qid,
		   ...
		   struct rte_mempool *mb_pool)
{
	...
	sfc_log_init(sa, "RxQ=%u nb_rx_desc=%u socket_id=%u",
		     ethdev_rx_qid, nb_rx_desc, socket_id);
	...
	dev->data->rx_queues[ethdev_rx_qid] = rxq_info->dp;
...
static int
sfc_tx_queue_setup(struct rte_eth_dev *dev, uint16_t ethdev_tx_qid,
		   ...
		   const struct rte_eth_txconf *tx_conf)
{
	...
	sfc_log_init(sa, "TxQ = %u, nb_tx_desc = %u, socket_id = %u",
		     ethdev_tx_qid, nb_tx_desc, socket_id);

	sfc_adapter_lock(sa);

	sw_index = sfc_txq_sw_index_by_ethdev_tx_qid(sas, ethdev_tx_qid);
	rc = sfc_tx_qinit(sa, sw_index, nb_tx_desc, socket_id, tx_conf);
	if (rc != 0)
		goto fail_tx_qinit;

	txq_info = sfc_txq_info_by_ethdev_qid(sas, ethdev_tx_qid);
	dev->data->tx_queues[ethdev_tx_qid] = txq_info->dp;
...
sfc_check_scatter_on_all_rx_queues(struct sfc_adapter *sa, size_t pdu)
{
	...
	for (i = 0; i < sas->ethdev_rxq_count; i++) {
		...
		if ((rxq_info->state & SFC_RXQ_INITIALIZED) == 0)
			continue;

		scatter_enabled = (rxq_info->type_flags &
				   EFX_RXQ_FLAG_SCATTER);

		rxq = sfc_rxq_ctrl_by_ethdev_qid(sa, i);
		if (!sfc_rx_check_scatter(pdu, rxq->buf_size,
 ...
 */
static void
sfc_rx_queue_info_get(struct rte_eth_dev *dev, uint16_t ethdev_rx_qid,
 ...
 */
static void
sfc_tx_queue_info_get(struct rte_eth_dev *dev, uint16_t ethdev_tx_qid,
		      struct rte_eth_txq_info *qinfo)
{
	struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
	struct sfc_txq_info *txq_info;

	SFC_ASSERT(ethdev_tx_qid < sas->ethdev_txq_count);
 ...
 */
static uint32_t
sfc_rx_queue_count(struct rte_eth_dev *dev, uint16_t ethdev_rx_qid)
{
	const struct sfc_adapter_priv *sap = sfc_adapter_priv_by_eth_dev(dev);
	struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
	struct sfc_rxq_info *rxq_info;
...
static int
sfc_rx_queue_start(struct rte_eth_dev *dev, uint16_t ethdev_rx_qid)
{
	struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
	struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
	struct sfc_rxq_info *rxq_info;
	...
	sfc_log_init(sa, "RxQ=%u", ethdev_rx_qid);
...
static int
sfc_rx_queue_stop(struct rte_eth_dev *dev, uint16_t ethdev_rx_qid)
{
	struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
	struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
	struct sfc_rxq_info *rxq_info;
...
static int
sfc_tx_queue_start(struct rte_eth_dev *dev, uint16_t ethdev_tx_qid)
{
	struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
	struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
	struct sfc_txq_info *txq_info;
	...
	sfc_log_init(sa, "TxQ = %u", ethdev_tx_qid);
...
static int
sfc_tx_queue_stop(struct rte_eth_dev *dev, uint16_t ethdev_tx_qid)
{
	struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
	struct sfc_adapter *sa = sfc_adapter_by_eth_dev(dev);
	struct sfc_txq_info *txq_info;
...
static int
sfc_rx_queue_intr_enable(struct rte_eth_dev *dev, uint16_t ethdev_rx_qid)
{
	const struct sfc_adapter_priv *sap = sfc_adapter_priv_by_eth_dev(dev);
	struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
	struct sfc_rxq_info *rxq_info;
...
static int
sfc_rx_queue_intr_disable(struct rte_eth_dev *dev, uint16_t ethdev_rx_qid)
{
	const struct sfc_adapter_priv *sap = sfc_adapter_priv_by_eth_dev(dev);
	struct sfc_adapter_shared *sas = sfc_adapter_shared_by_eth_dev(dev);
	struct sfc_rxq_info *rxq_info;
