...
sfc_rx_qflush(struct sfc_adapter *sa, unsigned int sw_index)
{
	...
	int ethdev_qid = sfc_ethdev_rx_qid_by_rxq_sw_index(sas, sw_index);
	     ...
	     struct rte_mempool *mb_pool)
{
	...
	if (!sfc_rx_check_scatter(sa->port.pdu, buf_size,
		...
		sfc_err(sa,
			"RxQ %u (internal %u) calculated Rx buffer size is %u vs "
			   ...
			   unsigned int ethdev_qid)
{
	...
	SFC_ASSERT(ethdev_qid < sas->ethdev_rxq_count);
...
struct sfc_rxq *
sfc_rxq_ctrl_by_ethdev_qid(struct sfc_adapter *sa, unsigned int ethdev_qid)
{
	struct sfc_adapter_shared *sas = sfc_sa2shared(sa);
	unsigned int sw_index;
