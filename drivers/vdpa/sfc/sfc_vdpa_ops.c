...
//#define SFC_GET_VI_INDEX(vq_num) ((vq_num) + 1)
...
sfc_vdpa_configure(struct sfc_vdpa_ops_data *vdpa_data)
{
	...
	SFC_EFX_ASSERT(vdpa_data->state == SFC_VDPA_STATE_INITIALIZED);
...
sfc_vdpa_enable_vfio_intr(struct sfc_vdpa_ops_data *vdpa_data)
{
	...
	for (i = 0; i < num_vring; i++) {
		...
		printf("\n virtqueue # %d, vring.callfd : %d", i, vring.callfd);
	}
...
sfc_vdpa_virtq_init(struct sfc_vdpa_ops_data *vdpa_data)
{
	...
	for (i = 0; i < vdpa_data->num_vring; i++) {
		...
		vq_cfg.evvc_msix_vector = RTE_INTR_VEC_RXTX_OFFSET + i;
		printf("\n vq_cfg.evvc_msix_vector : %d", vq_cfg.evvc_msix_vector);
		...
		printf("\n efx_virtio_virtq_create : rc %d", rc);
		if(rc == 0) {
			printf("\n virtqueue created sucessfully \n");
...
sfc_vdpa_close(struct sfc_vdpa_ops_data *vdpa_data)
{
	...
	SFC_EFX_ASSERT(vdpa_data->state == SFC_VDPA_STATE_STARTED);
	...
	/* TODO: Bypassed filter remove due to issue FWRIVERHD-940 
	   It will be enabled once EVb support would be available */
	#ifdef USE_FILTER_CONFIG
	printf("Remove installed MAC unicast and broadcast filters");
...
sfc_vdpa_start(struct sfc_vdpa_ops_data *vdpa_data)
{
	...
	SFC_EFX_ASSERT(vdpa_data->state == SFC_VDPA_STATE_CONFIGURED);
	...
	printf("\n From rte_vhost_get_negotiated_features (req_features) : %llx \n\n ", (long long unsigned int)vdpa_data->req_features);
	printf("\n drv_features : %llx \n\n ", (long long unsigned int)vdpa_data->drv_features);
...
#if 1
	printf("\n ->>>>For vq : %d", i); 
	printf("\n vq.desc (from rte_vhost_get_vhost_vring) :: %" PRIu64 "\n", (uint64_t)(uintptr_t)vq.desc);
	printf(" vq.desc (gpa) :: %" PRIu64 "\n", gpa);
#endif
...
#if 1
	printf("\n vq.avail (from rte_vhost_get_vhost_vring) :: %" PRIu64 "\n", (uint64_t)(uintptr_t)vq.avail);
	printf(" vq.avail (gpa) :: %" PRIu64 "\n", gpa);
#endif
...
#if 1
	printf("\n vq.used (from rte_vhost_get_vhost_vring) :: %" PRIu64 "\n", (uint64_t)(uintptr_t)vq.used);
	printf(" vq.used (gpa) :: %" PRIu64 "\n", gpa);
#endif
...
sfc_vdpa_dev_config(int vid)
{
	...
	#ifdef USE_FILTER_CONFIG
	printf("configure MAC unicast and broadcast filters");
	...
	if (rte_vhost_host_notifier_ctrl(vid, true) != 0)
	    ...
	    printf("\n rte_vhost_host_notifier_ctrl .. passed ... ");
...
sfc_virtio_proxy_verify_features(efx_nic_t *enp,
				unsigned int pf_index, unsigned int vf_index,
				efx_virtio_device_type_t type, uint64_t negotiated_features)
{
	int rc;
	efx_dword_t *proxy_hdr = NULL;
	size_t request_size = 0;
	...
	MCDI_IN_SET_DWORD(req, VIRTIO_TEST_FEATURES_IN_FEATURES_LO,
		negotiated_features & 0xFFFFFFFF);
	MCDI_IN_SET_DWORD(req, VIRTIO_TEST_FEATURES_IN_FEATURES_HI,
		((negotiated_features >> 32) & 0xFFFFFFFF));
...
sfc_vdpa_get_vfio_group_fd(int vid)
{
	...
	printf("\n vdpa_data->vfio_group_fd  : %d", vdpa_data->vfio_group_fd );
...
sfc_vdpa_get_vfio_device_fd(int vid)
{
	...
	printf("\n vdpa_data->vfio_dev_fd  : %d", vdpa_data->vfio_dev_fd );
...
sfc_vdpa_get_notify_area(int vid, int qid, uint64_t *offset, uint64_t *size)
{
	...
	printf("\n qid : %d, evv_data.evv_vi_index : %d", qid, evv_data.evv_vi_index);
	...
	printf("(qid modulo 2) : %d ", (qid % 2));
	if (qid % 2) { /* Even Id for RX and odd for TX */
		printf("\n Notify doorbell for  virtqueue # %d (TXQ)", qid);
		evv_data.evv_type = EFX_VIRTIO_VQ_TYPE_NET_TXQ;
	}
	else {
		evv_data.evv_type = EFX_VIRTIO_VQ_TYPE_NET_RXQ;
		printf("\n Notify doorbell for  virtqueue # %d (RXQ)", qid);
	}
	...
	printf("\n bar_offset (retrieved using libefx API): %d", bar_offset);

 	reg.index = vdpa_data->esb_rid;
	printf("\n reg.index (VFIO_DEVICE_GET_REGION_INFO): %d", (int)reg.index);
        ret = ioctl(vdpa_data->vfio_dev_fd, VFIO_DEVICE_GET_REGION_INFO, &reg);
        if (ret) {
                printf( "Could not get device region info: %s",strerror(errno));
                return -1;
        }
	printf("\n reg.offset (From VFIO_DEVICE_GET_REGION_INFO): %"PRIu64" ", (uint64_t)reg.offset);

	*offset = reg.offset + bar_offset;
	printf("\n\n Notify offset (reg.offset + bar_offset):: %" PRIu64 "\n\n", *offset);
