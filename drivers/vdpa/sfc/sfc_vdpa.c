...
#define MIN_VI_COUNT	4
#define MAX_VI_COUNT	4
...
static efx_rc_t
sfc_vdpa_find_mem_bar(efsys_pci_config_t *configp, int bar_index,
                 efsys_bar_t *barp)
{
        efsys_bar_t result;
...
static efx_rc_t
sfc_vdpa_pci_config_readd(efsys_pci_config_t *configp, uint32_t offset,
                     efx_dword_t *edp)
{
        int rc;
...
sfc_vdpa_mem_bar_init(struct sfc_vdpa_adapter *sva, const efx_bar_region_t *mem_ebrp)
{
	...
	printf("\n ebp->esb_base : %"PRIu64"", ((uint64_t)ebp->esb_base));

	printf("\n mem_ebrp->ebr_index : %d", mem_ebrp->ebr_index);
	printf("\n ebp->esb_rid  : %d", ebp->esb_rid );

	sva->vdpa_data->fcw_offset = mem_ebrp->ebr_offset;
	sva->vdpa_data->esb_base = ((uint64_t)sva->mem_bar.esb_base);
	sva->vdpa_data->esb_rid = sva->mem_bar.esb_rid;

	printf("\n sva->vdpa_data->esb_base : %"PRIu64"", ((uint64_t)sva->vdpa_data->esb_base));
	printf("\n sva->vdpa_data->esb_rid : %d", sva->vdpa_data->esb_rid);
...
sfc_vdpa_device_init(struct sfc_vdpa_adapter *sva)
{
	...
	efsys_pci_config_t espc;
  	static const efx_pci_ops_t ops = {
                .epo_config_readd = sfc_vdpa_pci_config_readd,
                .epo_find_mem_bar = sfc_vdpa_find_mem_bar,
        };
...
{
	SFC_EFX_ASSERT(sfc_vdpa_adapter_is_locked(sva->vdpa_data));
	...
	struct rte_pci_device *pci_dev)
{
	...
	/* TODO: Do not probe if MAC addr is not specified. Revisit: Mandatory/optional */
	...
	/* TODO : Move parent PF indentification in a seperate function */
	...
	/* Uate vdpa context vdpa_data fields */
	...
	sva->dev_addr.type = VDPA_ADDR_PCI;
	sva_list->sva = sva;
