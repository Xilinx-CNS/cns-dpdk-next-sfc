...
static int get_ep_dst(int bus, int dev)
{
	DIR *dp;
	struct dirent *ep;
	char *base = "/sys/bus/rpmsg/devices/";
...
int main(int argc, char **argv)
{
	...
	/*Create end points*/
	if (op == 1) {
		if ((fd = open("/dev/rpmsg_ctrl0", O_RDWR)) < 0) {
		...
		for(i = 0; i < gl.gl_pathc; i++) {
			...
			token1 = strtok(buf, ":");//first token is cdx_dev and ignored
			token1 = strtok(NULL, ":");
			token2 = strtok(NULL, ".");
		...
		for (i = 0; i < (gl.gl_pathc - 1); i++) {
			sprintf(dev_path, "/dev/rpmsg%d", i);
